...
void RearrangeCars::bruteForceAllSequence(vector<int>& currentStates, const vector<int>& endStates, vector<int>& carPosition, int emptySlotId, vector<Move>& sequenceOfMoves, vector<vector<Move>>& sequencesOfMoves, set<vector<int>>& usedPositions) {
	int numOfCarsOnDisiredPosition = numberOfCarsOnDesiredPosition(carPosition, endStates);
	if (numOfCarsOnDisiredPosition == carPosition.size()) {
		sequencesOfMoves.push_back(sequenceOfMoves);
		return;
	}
	for (int currCarSlotId = 0; currCarSlotId < carPosition.size(); currCarSlotId++) {
		if (currCarSlotId != emptySlotId) {
			moveCar(currentStates, carPosition, currCarSlotId, emptySlotId);
...
void RearrangeCars::generateAllSequencesOfMoves(const vector<int> &endStates, vector<vector<Move>> &sequencesOfMoves) {
	...
	bruteForceAllSequence(currentStates, endStates, carPosition, emptySlotId, sequenceOfMoves, sequencesOfMoves, usedPositions);
}
